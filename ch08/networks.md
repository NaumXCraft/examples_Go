...
```
## **Таблица: основные протоколы L2 - канального уровня OSI**

| Категория                      | Протоколы                                                             | Описание                                          |
| ------------------------------ | --------------------------------------------------------------------- | ------------------------------------------------- |
| **Ethernet (802.3)**           | Ethernet, Fast Ethernet, Gigabit Ethernet, 10G/40G/100G/400G Ethernet | Самые распространённые L2 сети; кадры, MAC-адреса |
| **Wi-Fi (802.11)**             | 802.11a/b/g/n/ac/ax/be, 802.11i, 802.11e, 802.11r                     | Беспроводные сети WLAN                            |
| **VLAN/QoS/Коммутация**        | 802.1Q, 802.1p, Q-in-Q, STP (802.1D), RSTP, MSTP, LACP (802.1AX)      | VLAN, spanning-tree, QoS, агрегация               |
| **Служебные L2 протоколы**     | LLDP, CDP, ARP*, GARP, GVRP, MACsec                                   | Автоопределение соседей, защита, управление       |
| **PPP семейство**              | PPP, PPPoE, PPPoA, MLPPP                                              | Каналы «точка-точка»                              |
| **Классические WAN-протоколы** | HDLC, Frame Relay, ATM                                                | Использовались в старых операторских WAN          |
| **MPLS (между L2/L3)**         | MPLS, EoMPLS                                                          | Передача пакетов по ярлыкам                       |
| **Беспроводные PAN (802.15)**  | Bluetooth (L2CAP), ZigBee (802.15.4), Thread                          | Личные сети (датчики, IoT, гарнитуры)             |
| **Операторские L2 протоколы**  | DOCSIS, GPON/EPON, Metro Ethernet (MEF)                               | Интернет-провайдеры, оптика                       |
| **Прочие L2 протоколы**        | Token Ring, FDDI                                                      | Исторические/устаревшие                           |

* ARP — протокол между слоями L2–L3 (Link + Network).
...
| **Категория**                              | **Протокол**      | **Описание**                                                                      |
| ------------------------------------------ | ----------------- | --------------------------------------------------------------------------------- |
| **Маршрутизация (динамические протоколы)** | **RIP / RIPv2**   | Простой протокол маршрутизации по количеству переходов.                           |
|                                            | **OSPF / OSPFv3** | Маршрутизация на основе состояния канала, используется в средних и больших сетях. |
